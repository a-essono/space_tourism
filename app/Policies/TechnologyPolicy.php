<?php

namespace App\Policies;

use App\Models\Technology;
use App\Models\User;
use Illuminate\Auth\Access\Response;

class TechnologyPolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        return $user->hasRole(['admin', 'technologies_admin']) && $user->can('technologies.view');
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, Technology $technology): bool
    {
        return $user->hasRole(['admin', 'technologies_admin']) && $user->can('technologies.view');
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return $user->hasRole(['admin', 'technologies_admin']) && $user->can('technologies.create');
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, Technology $technology): bool
    {
        return $user->hasRole(['admin', 'technologies_admin']) && $user->can('technologies.edit');
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, Technology $technology): bool
    {
        return $user->hasRole(['admin', 'technologies_admin']) && $user->can('technologies.delete');
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, Technology $technology): bool
    {
        return false;
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, Technology $technology): bool
    {
        return false;
    }
}
