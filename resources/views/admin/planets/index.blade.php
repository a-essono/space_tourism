                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($planets as $planet)
                            <tr class="whitespace-nowrap">
                                ...
                                <td class="px-4 py-4">
                                    <span
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $planet ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        {{ $planet ? 'Créée' : 'A créer' }}
                                    </span>
                                </td>
                                <td class="px-4 py-4 flex justify-center space-x-2">
                                    <x-link-button href="{{ route('fr.planete', $planet) }}"
                                        class="text-blue-600 hover:text-blue-400 hover:bg-gray-100">
                                        Voir
                                    </x-link-button>
                                    <x-link-button href="{{ route('admin.planetes.edit', $planet) }}"
                                    ...
                                    <x-link-button delete-cursor
                                        ...
                                        onclick="if (confirm('Voulez-vous vraiment supprimer cette planète ?')) {event.preventDefault(); document.getElementById('destroy{{ $planet->id }}').submit();}">
                                        Supprimer
                                    </x-link-button>
                                    <form id="destroy{{ $planet->id }}"
                                        action="{{ route('admin.planetes.destroy', $planet->id) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
