...
@endif
    <form action="{{ route('admin.planetes.store') }}" method="post">
        ...
        <div>
            ...
            <input type="text" step="any" id="distance_fr" name="distance_fr" class="w-full px-3 py-2 border rounded" value="{{ old('distance_fr') }}" required>
        </div>
        <!-- Distance (EN) -->
        <div>
            <x-label for="distance_en" label="Distance (Anglais)"/>
            <input type="text" step="any" id="distance_en" name="distance_en" class="w-full px-3 py-2 border rounded" value="{{ old('distance_en') }}" required>
        </div>

        <!-- Durée (FR) -->
        <div>
            <x-label for="duree_fr" label="Durée (Français)"/>
            <input type="text" step="any" id="duree_fr" name="duree_fr" class="w-full px-3 py-2 border rounded" value="{{ old('duree_fr') }}" required>
        </div>
        <!-- Durée (EN) -->
        <div>
            <x-label for="duree_en" label="Durée (Anglais)"/>
            <input type="text" step="any" id="duree_en" name="duree_en" class="w-full px-3 py-2 border rounded" value="{{ old('duree_en') }}" required>
        </div>
