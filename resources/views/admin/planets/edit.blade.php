<x-layout>
    <x-slot:title>
        Modifier la planète : {{ $planete->nom_fr }}
    </x-slot:title>
    ...
    <form action="{{ route('planetes.update', $planete->id) }}" method="POST">
        @csrf
        @method('PUT')
        <!-- Nom (FR) -->
        <div>
        ...
        <div>
            ...
            <input type="number" step="any" id="distance_fr" name="distance_fr" min="0" class="w-full px-3 py-2 border rounded" value="{{ old('distance_fr') }}" required>
        </div>
        <!-- Distance (EN) -->
        <div>
            <x-label for="distance_en" label="Distance (Anglais)"/>
            <input type="number" step="any" id="distance_en" name="distance_en" min="0" class="w-full px-3 py-2 border rounded" value="{{ old('distance_en') }}" required>
        </div>

        <!-- Durée (FR) -->
        <div>
            <x-label for="duree_fr_fr" label="Durée (Français)"/>
            <input type="number" step="any" id="duree_fr" name="duree_fr" min="0" class="w-full px-3 py-2 border rounded" value="{{ old('duree_fr') }}" required>
        </div>
        <!-- Durée (EN) -->
        <div>
            <x-label for="duree_en" label="Durée (Français)"/>
            <input type="number" step="any" id="duree_en" name="duree_en" min="0" class="w-full px-3 py-2 border rounded" value="{{ old('duree_en') }}" required>
        </div>
