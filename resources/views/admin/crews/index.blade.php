                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($crews as $crew)
                            <tr class="whitespace-nowrap">
                                ...
                                <td class="px-4 py-4">
                                    <span
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $crew ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        {{ $crew ? 'Créée' : 'A créer' }}
                                    </span>
                                </td>
                                <td class="px-4 py-4 flex justify-center space-x-2">
                                    <x-link-button href="{{ route('fr.equipage', $crew) }}"
                                        class="text-blue-600 hover:text-blue-400 hover:bg-gray-100">
                                        Voir
                                    </x-link-button>
                                    <x-link-button href="{{ route('admin.equipes.edit', $crew) }}"
                                    ...
                                    <x-link-button delete-cursor
                                        ...
                                        onclick="if (confirm('Voulez-vous vraiment supprimer ce membre de notre équipage ?')) {event.preventDefault(); document.getElementById('destroy{{ $crew->id }}').submit();}">
                                        Supprimer
                                    </x-link-button>
                                    <form id="destroy{{ $crew->id }}"
                                        action="{{ route('admin.equipes.destroy', $crew->id) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
